...
| **Aspect**                       | **Findings**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Algorithm Performance**        | - **SVGD**: Consistently performed well across different tasks, when using for example the adam optimizer, but it also gets stuck quite easily when using the optax optimizer SGD(). Performs better than random forest most of the time for especially for classification problem, but for example also for the diabetes dataset. <br>- **sSVGD**: Introduced beneficial Noise, but also gets stuck pretty easily, when using the SGD optimizer. <br> Also the Problem arises, that the noise is very small compared to the particle coordinates so the noise does not really effect the training and one just gets the same result as when using svgd with the SGD optimizer. One can get different/better results for sSVGD when we multiply the noise with a constant factor e.g. 1000 when adding the noise in Algorithm 2 "A stochastic Stein Variational Netwon method" by Alex Leviyev et. all. Here on need to do that manually in the local blackjax file, when using SGD. <br> Also when using for example the adam optimizer as a optimizer for sSVGD one get small differences in the performance compared to using SVGD, but sometimes noise introduces to much uncertainty so that the algorithms does find the optimum. |
| **Number of Particles**          | - Increasing the number of particles improved posterior approximation but with diminishing returns beyond a certain point (e.g., 50 particles).<br>- More particles increased computational load immensely.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **Batch Size**                   | - Smaller data batch sizes introduced more noise but helped in escaping local minima, but also to much noise i.e. when the batch size is to small, can have negative effects and result in slow or no convergence. <br>- Particle mini-batching helped manage memory usage when using a large number of particles, also increases computation time. Only a small effect on the MSE and accuracy score was visible sometimes, so no clear statement can be made.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **Learning Rate and Optimizers** | - Adaptive optimizers like Adam performed much better than plain SGD. SGD often gets stuck for example for the MNIST dataset roughly at a accuracy of 10% so the results are really bad. Adam on the other hand can reach a accuracy of up to 98% for the MNIST dataset.<br>- Learning rate schedules (e.g., exponential decay) improved training stability over long runs, here a problem arises when using batching of the data since the decay rate is not applied on the basis of the epochs, so the decay is much stronger, one can adjust for that by just increasing the transitioning steps or decreasing the decay rate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Kernel Parameters in SVGD**    | - The kernel length scale significantly impacted particle updates. The best kernel length strongly depends on the used dataset .<br>- Using median heuristics to initialize the kernel length scale yielded good results.<br>- Alternative kernels could be explored for better performance.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Training Dynamics**            | - Early iterations showed rapid decreases in loss/accuracy, with slower improvements over time.<br>- sSVGD displayed more fluctuations due to stochasticity when trained with ADAM optimizer. When using SGD() the fluctuations are not that visable since the model gets stuck pretty quickly. The fluctuation depends of course strongly on the learning, the decay rate and the transitioning steps                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
