...
def set_up_ssvgd(steinvi_ssvdg):
    """Sets up the stochastic Stein Variational Gradient Descent (sSVGD)
    process for training a Bayesian Neural Network (BNN).

    This function initializes the state for SVGD, including defining the update function for sSVGD
    and the evaluation function for assessing the model's performance.
    This function performs sSVGD. Here we often get the same results as in normal SVGD this is because the particles
    weights are very big values e.g. between -40000 and 40000
    and the variance of the noise is based of the Kernel which only can take on values between 0 and 1
    so the noise is very small. By adding a constant factor to the noise in the local blackjax file
    e.g. noise * 1000000, instead of jnp.sqrt(learning_rate), one can get different results to SVGD.
...
def initialize_ssvgd_state(steinvi_ssvdg):
    ...
    Args:
        steinvi_ssvdg (SteinVI_BNN): stein_vi object for sSVGD algorithm
    ...
    ssvgd = stein_vi.algorithm.sSVGD.local_blackjax_file_with_adjustments_for_sSVGD.as_top_level_api(grad_log_posterior,
                                                                                                     steinvi_ssvdg.parameter.optimizer,
                                                                                                     rbf_kernel,
...
def particle_minibatching(state, z_batch, y_batch, step_fn, particle_indices):
    ...
    Args:
        state (sSVGDState): The current SVGD state containing particles and optimizer state.
...
def update_optimizer_state(optimizer_state, batched_state, indices):
    ...
    Args:
        ...
        batched_state (sSVGDState): The optimizer state after a minibatch update.
