...
@jax.jit
def logp_unnormalized_posterior_regession(x, input_1, input_2, true_output, prior_mu, nnet_model, treedef):
...
def logp_unnormalized_posterior(params, dz, dy, nnet_model, treedef):
    # Calculate log prior using Gamma distribution
    log_prior = jnp.sum(norm.logpdf(params, loc=0,
                                    scale=1))
    # jnp.sum(stats.norm.logpdf(params, prior_mu, prior_prec))#jnp.sum(gamma.logpdf(params, a=prior_mu, scale=1/prior_prec))
