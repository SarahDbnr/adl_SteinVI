...
def calculate_mse(nnet_model, treedef, random_input_1, random_input_2, state, mean_squared_errors):
    ...
    prediction = jax.vmap(lambda p: nnet_model.apply(treedef(p), random_input_1, random_input_2))(state.particles).mean(0)
...
def plot_and_save_accuracy(val_accuracies, num_particles, network_structure, kernel_length, adam_learning_rate, warm_up_iterations, output_folder="plots"):
