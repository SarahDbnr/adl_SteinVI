...
from local_SVGD_Stepwise_build import rbf_kernel, update_median_heuristic, as_top_level_api_stochastic, as_top_level_api
import local_SVGD_Stepwise_build
...
def train_with_svgd(dataset, output_size, network_structure, batch_size, num_particles, key, regression, pen_lambda=0.01):
    ...
    out, evaluation_metrics_1, evaluation_metrics_2 = ssvgd_training_loop(
        log_p=logp_model,
...
def svgd_training_loop(
    ...
    svgd =  as_top_level_api(grad_log_posterior, optimizer, kernel, update_median_heuristic)
...
def ssvgd_training_loop(
    ...
    grad_log_posterior = jax.grad(log_p)
    ssvgd =  as_top_level_api_stochastic(grad_log_posterior, optimizer, kernel, update_median_heuristic)
    state = ssvgd.init(initial_position, initial_kernel_parameters)
    step = jax.jit(ssvgd.step)
